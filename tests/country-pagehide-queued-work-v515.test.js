...
test("Country Settings invalidates queued render and relabel work on pagehide", () => {
  assert.match(source, /let pageActive = true/);
  assert.match(source, /pageActive = false;\s*renderGeneration \+= 1;\s*renderQueued = false;\s*relabelQueued = false;/s);
  ...
  assert.match(source, /function runPersonalListRelabel\(\) \{\s*relabelQueued = false;\s*if \(!pageActive\) return;/s);
