...
test("M398 revoked raw cache roots fail through the reviewed cache boundary", () => {
  assert.throws(() => assertRawListCacheBound(revokedProxy({})), /inspectable array kind/);
  assert.throws(() => assertRawListCacheBound(revokedProxy([])), /inspectable array kind/);
});

test("M398 revoked nested cache JSON values fail deterministically", () => {
  assert.throws(
...
test("M398 ordinary cache JSON semantics remain available", () => {
