...
test("M456 direct message validator rejects invalid groups before inspecting message", () => {
  let typeReads = 0;
  const message = {};
  ...
  assert.throws(
    () => validateBackgroundRuntimeMessage(message, "other"),
    /Message guard group is invalid/
  );
  assert.equal(typeReads, 0);
  assert.throws(() => validateBackgroundRuntimeMessage({}, new String("core")), /Message guard group is invalid/);
});

test("M456 valid groups retain cross-group handled false behavior", () => {
  assert.deepEqual(
